...
// NamedAttacher optionally receives the user and attach names supplied by
// Tattach. Servers that need attach-name authorization can implement this in
// addition to Attacher; existing Attacher implementations remain compatible.
type NamedAttacher interface {
	AttachName(userName, attachName string, uid UID) (File, error)
}
