...
func (t *tattach) handle(cs *connState) message {
	...
	// Do the attach on the root. Preserve the original attach name for
	// optional capability-aware attachers before using it as a walk path.
	attachName := t.Auth.AttachName
	var sf File
	var err error
	if named, ok := cs.server.attacher.(NamedAttacher); ok {
		sf, err = named.AttachName(t.Auth.UserName, attachName, t.Auth.UID)
		// A NamedAttacher consumes aname as authorization/application data,
		// rather than interpreting it as an attach-time filesystem walk.
		t.Auth.AttachName = ""
	} else {
