...
impl DeployPipeline {
    ...
    pub fn artifact_store(&self) -> DeployBlobStore {
            ...
            FunctionRuntime::Bun => match String::from_utf8(artifact) {
                ...
                Err(error) => {
                    return failed(
                        job,
                        Instant::now(),
                        format!("Bun artifact is not UTF-8: {error}"),
                    );
                }
            },
        }
...
mod tests {
    ...
    async fn pipeline(dir: &TempDir) -> DeployPipeline {
        DeployPipeline::open(dir.path().join("fn"), Namespace::new())
            .await
            .unwrap()
    }
    ...
    async fn deploy_registers_manifest_in_namespace() {
        ...
        let p = DeployPipeline::open(dir.path().join("fn"), ns.clone())
            .await
            .unwrap();
        let job = DeployJob::new(
            "greet",
        ...
        assert!(
            ns.get(&format!(
                "/deploy/blobs/{}",
                manifest["artifact_cid"].as_str().unwrap()
            ))
            .await
            .is_some(),
    ...
    async fn materializes_a_routed_artifact() {
        ...
        let cid = p
            ...
            .unwrap();
        let path = p
            .materialize("hello", &FunctionRuntime::Bun, &cid)
            .await
            .unwrap();
        assert_eq!(
