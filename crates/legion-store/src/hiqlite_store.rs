...
use hiqlite::{Client, Node, NodeConfig, macros::params};
...
use legion_core::{
    ...
    traits::EventStore,
    types::{
        RunConfig, RunId, SeqNum, SessionFilter, SessionStatus, SessionSummary, TurnEnvelope,
...
impl HiqliteStore {
    ...
    pub async fn connect(config: NodeConfig) -> Result<Self> {
        let api_addresses = config
            ...
            .collect();
        let api_secret = config.secret_api.clone();
    ...
    pub fn from_client(client: Client) -> Self {
        Self {
            client: Arc::new(client),
            api_addresses: Arc::new(Vec::new()),
            api_secret: Arc::new(String::new()),
        }
    }

    /// Broadcast a durable session wake-up notification through hiqlite.
    pub async fn notify_session(&self, run_id: RunId) -> Result<()> {
        self.write_client()
            .await?
            .notify(&run_id.to_string())
            .await
            .map_err(|error| LegionError::Store(format!("notify session: {error}")))
    }

    /// Wait for the next local or replicated wake-up for a session.
    pub async fn wait_for_session(&self, run_id: RunId) -> Result<()> {
        let expected = run_id.to_string();
        loop {
            let (_, notified): (i64, String) = self
                .client
                .listen()
                .await
                .map_err(|error| LegionError::Store(format!("listen session: {error}")))?;
    ...
    /// Acquire a cluster-wide lease for leader-only maintenance work.
    pub async fn leader_lock(&self, operation: &str) -> Result<hiqlite::Lock> {
        self.write_client()
            .await?
            .lock(format!("legion:{operation}"))
            .await
            .map_err(|error| LegionError::Store(format!("distributed lock: {error}")))
    }

    /// Expose the current Raft leader for health/namespace reporting.
    ...
    /// Return compact local Raft diagnostics for integration assertions.
    pub async fn raft_diagnostics(&self) -> String {
        match self.client.metrics_db().await {
            Ok(metrics) => format!(
                "id={} state={:?} leader={:?} running={:?} voters={:?}",
                metrics.id,
                metrics.state,
    ...
    /// Gracefully stop this embedded Raft node.
    pub async fn shutdown(&self) -> Result<()> {
        self.client
            .shutdown()
            .await
            .map_err(|error| LegionError::Store(format!("shutdown hiqlite node: {error}")))
    }

    /// Connect as an external client so leader changes are followed automatically.
    pub async fn remote(api_addresses: Vec<String>, api_secret: String) -> Result<Self> {
        let api_addresses: Vec<String> = api_addresses
            .into_iter()
            .map(|address| {
                address
                    .trim_start_matches("http://")
        ...
        .await
        .map_err(|error| LegionError::Store(format!("connect remote hiqlite client: {error}")))?;
        Ok(Self {
            client: Arc::new(client),
            api_addresses: Arc::new(api_addresses),
            api_secret: Arc::new(api_secret),
    ...
    async fn write_client(&self) -> Result<Client> {
        if self.api_addresses.is_empty() {
            return Ok((*self.client).clone());
        }
        Client::remote(
            self.api_addresses.as_ref().clone(),
            false,
            false,
            self.api_secret.as_ref().clone(),
            false,
            None,
            None,
        )
        .await
        .map_err(|error| LegionError::Store(format!("connect write client: {error}")))
    }
}
...
impl EventStore for HiqliteStore {
    async fn create_session(&self, run_id: RunId, config: &RunConfig) -> Result<()> {
        let config_json =
            serde_json::to_string(config).map_err(|e| LegionError::Serialization(e))?;
        let idle = serde_json::to_string(&SessionStatus::Idle)
            .map_err(|e| LegionError::Store(e.to_string()))?;
        let now = now_ms();

        self.write_client().await?.execute(
    ...
    async fn append(&self, run_id: RunId, event: TurnEvent) -> Result<SeqNum> {
        ...
        let kind_tag =
            serde_json::to_string(&event.kind).unwrap_or_else(|_| "\"SessionStarted\"".into());
    ...
    async fn read_recent(&self, run_id: RunId, n: usize) -> Result<Vec<TurnEnvelope>> {
        ...
        let mut envs: Vec<TurnEnvelope> = rows
            .into_iter()
            .map(|r| turn_row_to_envelope(run_id, r))
            .collect();
        envs.reverse();
        Ok(envs)
    }
    ...
    async fn session_status(&self, run_id: RunId) -> Result<SessionStatus> {
        ...
        serde_json::from_str(&s).map_err(|e| LegionError::Store(format!("deserialize status: {e}")))
    }

    async fn set_status(&self, run_id: RunId, status: SessionStatus) -> Result<()> {
        let s = serde_json::to_string(&status).map_err(|e| LegionError::Store(e.to_string()))?;
        let now = now_ms();
        self.write_client()
            .await?
            .execute(
            ...
            .await
            .map_err(|e| LegionError::Store(e.to_string()))?;
        self.notify_session(run_id).await?;
        Ok(())
    }
    ...
    async fn list_sessions(&self, filter: SessionFilter) -> Result<Vec<SessionSummary>> {
        ...
        rows.into_iter()
            .map(|r| {
                let run_id =
                    Uuid::parse_str(&r.run_id).map_err(|e| LegionError::Store(e.to_string()))?;
